import { deley, makePlayersQueye, sumQueueResults } from "./utils.ts";
import { join } from 'jsr:@std/path'
import puppeteer, { Browser } from 'npm:puppeteer'
import { install, getInstalledBrowsers,  Browser as InstallBrowser } from 'npm:@puppeteer/browsers'
...
export async function runBot(
  ...
  let cacheDir = join(Deno.cwd(), "browser");
  ...
  if(availableBrowsers.length === 0){ 
    ...
    cacheDir = executablePath
    console.log('Browser installed.'); 
  }else{
    cacheDir = availableBrowsers[0].executablePath
  }
	...
	for (const [index, players] of queue.entries()) {
		...
		try {
			...
			console.log("[Error] Error while redeeming the code for the player!");
		}
	}
